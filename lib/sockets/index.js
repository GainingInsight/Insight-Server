...
module.exports = function(io) {
    io.use(function(socket, next) {
        ...
        table.getItem({
            Key: { session_id: { S: token.sessionId } }
        }, function(err, data) {
            if(err) console.log(err, err.stack);
            else if(data) {
                socket.sessionId = token.sessionId;
                socket.sessionKey = token.sessionKey;
                socket.clientRole = (function() {
                    if(socket.sessionKey === data.Item.session_key_playerNS.S)
                        return 'playerNS';
                    else if(socket.sessionKey === data.Item.session_key_playerFS.S)
                        return 'playerFS';
                    else if(socket.sessionKey === data.Item.session_key_researcher.S)
                        return 'researcher';
                    else
                        return '';
                })();
    ...
    io.on('connection', function(socket) {
        ...
        socket.on(Messages.MOVEMENT_SET, function(data) {
            console.log('Movement message received from ' + socket.clientRole);

            socket.broadcast.emit(Messages.MOVEMENT, {
                clientRole: socket.clientRole,
                playerX: data.data.playerX,
                playerY: data.data.playerY,
                key: data.data.key,
                pressed: data.data.pressed
            });
        });
        ...
        socket.on(Messages.INITIATION, function(data) {
            ...
            table.getItem({
                ...
                if(err) console.log(err, err.stack);
                    ...
                    if(readyKeys.indexOf(sessionKey) > -1 && readyKeys.length > 2) {
                    ...
                    } else if(readyKeys.indexOf(sessionKey) === -1 && readyKeys.length > 1) {
