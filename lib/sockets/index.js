...
module.exports = function(io) {
    io.on('connection', function(socket) {
        ...
        socket.on(Messages.INITIATION, function(data) {
            ...
            redisClient.set(sessionId + ':' + sessionKey, socket.id);
            redisClient.set(socket.id, sessionId + ':' + sessionKey);
        ...
        socket.on('disconnect', function() {
            ...
            redisClient.get(socket.id, function(errRedis, dataRedis) {
                if(errRedis) {
                    console.log(errRedis, errRedis.stack);
                    return;
                } else if(!dataRedis) {
                ...
                var sessionCredentials = dataRedis.split(':');
                var sessionId = sessionCredentials[0];
                var sessionKey = sessionCredentials[1];
                ...
                table.getItem({
                    ...
                    if(err) console.log(err, err.stack);
                        ...
                        var readyKeys = data.Item.ready_keys.SS;
                        console.log('EE: ' + JSON.stringify(readyKeys, null, 4));
